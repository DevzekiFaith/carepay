    ...
    return (
        <div className="w-full max-w-md rounded-3xl border-2 border-stone-100 bg-white p-4 shadow-xl shadow-stone-200/20 sm:p-6">
            ...
            <div className="grid grid-cols-7 gap-1 text-center mb-4">
                {DAYS.map(day => (
                    <span key={day} className="text-xs font-black uppercase tracking-widest text-stone-400">{day}</span>
                ))}
            ...
            <div className="mt-8 border-t border-stone-50 pt-6">
                <p className="text-xs font-black uppercase tracking-widest text-stone-400 mb-4">Available Times</p>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                    {TIME_SLOTS.map(time => (
                        <button
                            key={time}
                            onClick={() => handleTimeClick(time)}
                            className={`rounded-xl border-2 py-3 text-xs font-black uppercase tracking-widest transition-all ${selectedTime === time
